...
<head>
    <meta charset="UTF-8">
    <title><?= htmlspecialchars($data['genre'] ?? '2genre') ?></title>
...
<body>
    ...
    <div class="hero" style="background-image: url('<?= $url_alias ?>/assets/img/boots_house.jpg');">
        <a href="<?= $url_alias ?>/" class="btn-home">← Home</a>

        <div class="hero-content">
            <h1><?= htmlspecialchars($data['genre'] ?? '2genre') ?></h1>
    ...
    <div class="grid-container">
        <div class="grid">
            <?php if (!empty($data['items'])): ?>
                ...
                <?php endforeach; ?>
                <?php else: ?>
                <p style="text-align:center; width:100%; grid-column: 1/-1;">Ainda não há músicas <?= htmlspecialchars($data['genre'] ?? '2genre') ?>.</p>
            <?php endif; ?>
        </div>
    </div>
